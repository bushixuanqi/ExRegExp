import { range } from "../op/opNumber.js"
import { tsCode } from "../op/opStr.js"
 ...
 * 上述两种插入到正则中的格式,可捕获正则匹配相应部分的结果,返回值以标记作为键名存入到标记值库
 * 后续[[mark]]可从标记值库中读取相应值插入到其所在位置代替占位标记,若标记值库中没有结果,而fns中有相应函数则无参调用这个函数返回值替换
 * 
 * [[mark]] //按标记插入对应标记的调用结果
 * 未存在于fns的函数的键,采用顶部声明的innerFn作为默认标记函数
 * 
...
//match2: ['kakaeffafa']
 */
export default function exRegExp({ reg, fns = {}, str, next }) {
    const oldSplit = /\[\[(.*?[^\\])\]\]/g;
    let regExp, notFirst, source, flags, newFlags, nextCall, res = {}, oldRes = {}, old = [];
    if (reg) {
        source = reg.source.replace(/\\\[(?=\[)/g, '(?:\\[)');
        if (!(newFlags = (flags = reg.flags).replace('y', 'g')).includes('g')) {
            newFlags += 'g';
        };
...
/**
 * 正则函数调用及函数结果插入递归替换
 * @param {{ oldRes:Any, source:String, flags:String, split:RegExp, res:Any, fns:Array<Function>, str:String, old:Array, lastIndex:Number, oldReg:RegExp, oldLeft:String, oldRight:String, inner:String, oldSource:String }} param0 
 * @returns 
 */
    ...
    if (oldReg) {//回溯处理
        ...
        switch (oldNum) {
            case 0:
                ...
                break;
            case 1:
                res[oldLeft] = (fns[oldRight] || innerFn)(mats, reg);
                ...
                if (mats) {//匹配成功递归处理
                    const index2 = first.indexOf('='),
                        [left, right] = index2 === -1 ? [first, first] : tsCode(first, [index2, 1, Infinity], {
                            0: true,
                            2: true
                        }), fnRes = (fns[right] || innerFn)(mats, reg);
                    res[left] = fnRes;
